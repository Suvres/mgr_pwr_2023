...
Jest to najprostsza odmiana sieci neuronowej, czyli pojedynczy perceptron, który jest matematycznym modelem neuronu. Składa się on z \textit{n} wejść, takiej samej ilości wag, progu $\Theta$, sumatora, funkcji aktywującej i wyjścia. Został zobrazowany na \refsource{schemacie}{fig:neuron}. Może służyć za prosty klasyfikator binarny albo za regresor. Model wykorzystany w Azure ML ukazano na \refsource{rysunku}{fig:ap-pipe}.
...
\subsection{DANet}
Twórcy tego algorytmu wprowadzają dodatkową warstwę abstrakcyjną o nazwie ,,\textit{Abstract Layer}''. Warstwy te budują sieć o nazwie ,,\textit{Deep Abstract Network}'' (DANet). Zadanie dodatkowych warstw jest grupowanie cech w skorelowanych zbiorach. Zbiory te budują sieć powiązań między sobą w formie sieci semantycznej. Gdy sieć semantyczna jest zbudowana, to w ostatnim kroku wykonywana jest klasyfikacja w trzywarstwowej sieci perceptronów \textit{(ang. Multilayer Perceptron network, MLP)}~\cite{Chen2022, Danet}. Sposób działania przedstawiono na \refsource{schemacie}{fig:danet-abst}. Model znajdujący się z Azure ML został przedstawiony na \refsource{rysunku}{fig:danet-pipe}.
...
\\ \\
Całe doświadczenie zostało odwzorowane w graficznym potoku narzędzia ,,\textit{Projektant}'' oraz przedstawione na \refsource{rysunku}{fig:pipeline}.
...
Kolejną grupą zadań widoczną w potoku są te związane z trenowaniem i testowaniem poszczególnych algorytmów opisanych w \refsource{rozdziale}{cha:dos}. Każdy test składa się 3 kafelków. W przypadku algorytmów dostarczonych wraz z platformą Azure ML są to:
\begin{itemize}
...
\begin{itemize}
    ...
    Korzystając ze statystycznego testu t~-~Studenta dla prób zależnych, wykonano obliczenia dla poszczególnych relacji przedstawionych w \refsource{tabeli}{tab:acc-p-stat}.
    \begin{equation}
        \label{eq:t-p1}
    ...
    \begin{itemize}
        ...
        \item $p3$ -- próba 3
    \end{itemize}
    \refsource{Tabela}{tab:p-val} przedstawia rozkład wartości \textit{t} dla \textit{n} punktów swobody w \textit{p-value}.
    \begin{table}[H]
        \centering
    ...
    \end{itemize}
    Wyniki równań zostały przedstawione w \refsource{tabeli}{tab:acc-p-stat} w kolumnie \textit{wartość t}. Wartość \textit{p-value} dla każdego porównania została opisana na podstawie \refsource{tabeli}{tab:p-val}.
    ...
    Przez to, że dane pomiędzy próbami testowymi nie posiadają różnic istotnych statystycznie, do poniższej analizy wykorzystano dane z próby testowej nr 1. Wykorzystując znów statystyczny test t~-~Studenta dla prób zależnych dla danych z \refsource{tabeli}{tab:acc-res} wyznaczono wartość:
    \begin{equation}
        \label{eq:test-t}
...
\end{itemize}
\ \\
Analiza wyników wykazała, że algorytm utworzony w ramach pracy inżynierskiej~\cite{Blyszcz2022}, wypada gorzej w porównaniu z algorytmami dostarczonymi przez platformę Microsoft Azure. Jego dokładność odstaje od dokładności uzyskanych przez pozostałe algorytmy. Zostało to przedstawione w \refsource{tabeli}{tab:acc-res}. Wyniki uzyskane przez algorytm \textit{Gaussian Naive Bayes - with GA} wynoszą $[72,4470\%; 73,8269\%; 76,2585\%]$. Wyniki te są słabsze o około 12-16 p.p. od wyniku uzyskanego przez algorytm DANet. Występująca różnica pomiędzy obecnymi wynikami GAGNB a wynikami uzyskanymi w pracy inżynierskiej (\refsource{sekcja}{sec:inz-rez-pod}) są związane z rozmiarem wykorzystanych danych testowych. W testach w ramach pracy inżynierskiej wykorzystywano kilka mniejszych zbiorów danych, symbolizujące poszczególne dni robocze pracy w biurze (\refsource{sekcja}{sec:data}). W ramach pracy magisterskiej, przez wzgląd na kosztowność przeprowadzanych doświadczeń, sprawdzono poszczególne algorytmy na pojedynczym zbiorczym pliku zawierającym złączone dane testowe.
\\ \\
